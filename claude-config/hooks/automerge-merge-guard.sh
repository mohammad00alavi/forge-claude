...
# Resolve THIS project's repo — the toggle being honored is this repo's.
# Unresolvable == fail closed.
REPO=$(gh repo view --json nameWithOwner -q .nameWithOwner 2>/dev/null)
[ -z "$REPO" ] && block "cannot resolve this project's GitHub repo, so the LOOP_AUTOMERGE toggle cannot be verified. Leave the PR ready for the human."
...
# Shape, toggle, scope, CI, labels, paths, freshness, feedback and the review
# verdict are all verified.
exit 0
